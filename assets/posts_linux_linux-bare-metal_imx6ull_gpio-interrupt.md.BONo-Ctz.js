import{_ as i,o as a,c as n,a2 as p}from"./chunks/framework.BsLG2nmw.js";const g=JSON.parse('{"title":"GPIO 中断实验","description":"","frontmatter":{"title":"GPIO 中断实验","category":"Linux","subCategory":"linux-bare","subSubCategory":"i.MX6ULL"},"headers":[],"relativePath":"posts/linux/linux-bare-metal/imx6ull/gpio-interrupt.md","filePath":"posts/linux/linux-bare-metal/imx6ull/gpio-interrupt.md"}'),l={name:"posts/linux/linux-bare-metal/imx6ull/gpio-interrupt.md"};function t(h,s,k,e,r,E){return a(),n("div",null,[...s[0]||(s[0]=[p(`<h1 id="gpio-中断实验" tabindex="-1">GPIO 中断实验 <a class="header-anchor" href="#gpio-中断实验" aria-label="Permalink to &quot;GPIO 中断实验&quot;">​</a></h1><p>中断系统是一个处理器重要的组成部分，中断系统极大的提高了 CPU 的执行效率</p><h1 id="correx-a7-中断详解" tabindex="-1">Correx A7 中断详解 <a class="header-anchor" href="#correx-a7-中断详解" aria-label="Permalink to &quot;Correx A7 中断详解&quot;">​</a></h1><p><strong>STM32 中断系统回顾</strong></p><p>STM32 的中断系统主要有以下几个关键点：</p><ul><li><p>中断向量表</p><p>中断向量表是一个表，这个表里面存放的是中断向量。中断服务程序的入口地址或存放中断服务程序的首地址成为中断向量，因此中断向量表是一系列中断服务程序入口地址组成的表。这些中断服务程序(函数)在中断向量表中的位置是由半导体厂商定好的，当某个中断被触发以后就会自动跳转到中断向量表中对应的中断服务程序(函数)入口地址处</p></li><li><p>NVIC(内嵌向量中断控制器)</p><p>中断系统得有个管理机构，对于 STM32 这种 Cortex-M 内核的单片机来说这个管理机构叫做 NVIC，全称叫做 Nested Vectored Interrupt Controller</p><p>Cortex-A 内核的中断管理机构不叫做NVIC，而是叫做 GIC，全称是 general interrupt controller</p></li><li><p>中断使能</p><p>要使用某个外设的中断，肯定要先使能这个外设的中断</p></li><li><p>中断服务函数</p><p>我们使用中断的目的就是为了使用中断服务函数，当中断发生以后中断服务函数就会被调用，我们要处理的工作就可以放到中断服务函数中去完成</p></li></ul><h1 id="硬件" tabindex="-1">硬件 <a class="header-anchor" href="#硬件" aria-label="Permalink to &quot;硬件&quot;">​</a></h1><p>和按键输入的所使用的一致</p><h1 id="实验程序编写" tabindex="-1">实验程序编写 <a class="header-anchor" href="#实验程序编写" aria-label="Permalink to &quot;实验程序编写&quot;">​</a></h1><h2 id="添加相应文件" tabindex="-1">添加相应文件 <a class="header-anchor" href="#添加相应文件" aria-label="Permalink to &quot;添加相应文件&quot;">​</a></h2><ul><li><p>复制上一节的文件夹</p><div class="language-shell vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">shell</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">cp</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> -rf</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> 8_clk/</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> 9_int/</span></span>
...
<span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">./imxdownload</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> int.bin</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> /dev/sdb</span></span></code></pre></div>`,29)])])}const y=i(l,[["render",t]]);export{g as __pageData,y as default};
